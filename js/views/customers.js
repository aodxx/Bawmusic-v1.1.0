...
function paintCustomers(list) {
  ...
  container.innerHTML = `
    ...
    <p class="text-xs text-gray-500 mb-2">${list.length} คน</p>
    ...
    <div class="bg-navy-light rounded-2xl p-3.5 border border-gold/10">
      <div class="flex items-center justify-between mb-2">
        <div class="flex items-center gap-2.5">
          <div class="w-9 h-9 rounded-full bg-gold/20 flex items-center justify-center text-gold text-sm font-semibold">
            ${(c.name || '?').charAt(0)}
          </div>
          <div>
            <p class="text-sm font-medium text-gray-100">${c.name}</p>
            <p class="text-[11px] text-gray-500">${c.phone || '-'} ${c.line ? '· ' + c.line : ''}</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-3 gap-2 text-center border-t border-gold/10 pt-2">
        <div>
          <p class="text-xs font-semibold text-gold">${c.totalBookings || 0}</p>
          <p class="text-[9px] text-gray-500">งานทั้งหมด</p>
        </div>
        <div>
          <p class="text-xs font-semibold text-gold">${Utils.formatMoney(c.totalRevenue)}</p>
          <p class="text-[9px] text-gray-500">รายได้รวม</p>
        </div>
        <div>
          <p class="text-xs font-semibold text-gold">${c.favoriteJobType ? Utils.jobTypeLabel(c.favoriteJobType) : '-'}</p>
          <p class="text-[9px] text-gray-500">งานที่ชอบ</p>
        </div>
      </div>
    </div>
  `;
