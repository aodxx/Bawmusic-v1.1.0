...
function paintBookings() {
  ...
  container.innerHTML = `
    ...
    <p class="text-xs text-gray-500 mb-2">${list.length} รายการ</p>
    <div class="space-y-2">
...
function filterChip(label, value, key) {
  ...
  return `
    <button onclick="window.__setBookingFilter('${key}','${value}')"
      class="px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0 transition-colors ${active ? 'bg-gold text-navy-dark' : 'bg-navy-light text-gray-400'}">
      ${label}
    </button>
  `;
}
...
function bookingRow(b) {
  return `
    <div class="bg-navy-light rounded-2xl p-3.5 border border-gold/10 active:scale-[0.98] transition-transform cursor-pointer"
         onclick="window.__app.openBookingForm('${b.id}')">
      ...
      <p class="text-xs text-gray-400 mb-1"><i class="fa-solid fa-location-dot mr-1 w-3"></i>${b.venue || 'ไม่ระบุสถานที่'} ${b.province ? '· ' + b.province : ''}</p>
