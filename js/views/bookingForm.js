  ...
  <div class="fixed inset-0 z-50 bg-navy-dark overflow-y-auto" id="booking-modal">
    ...
    <div class="px-4 py-4 max-w-2xl mx-auto space-y-4 pb-10">
      ...
      <div class="flex gap-2 overflow-x-auto pb-1">
        ${templates.map(t => `
          <button onclick='applyTemplate(${JSON.stringify(t).replace(/'/g, "&apos;")})'
            class="flex-shrink-0 px-3 py-2 bg-navy-light border border-gold/20 rounded-xl text-xs text-gray-200">
      ...
      <section class="bg-navy-light rounded-2xl p-4 border border-gold/10">
        <h3 class="text-xs font-semibold text-gold mb-3"><i class="fa-solid fa-user mr-1.5"></i>ข้อมูลลูกค้า</h3>
      ...
      <!-- Venue -->
      <section class="bg-navy-light rounded-2xl p-4 border border-gold/10">
        <h3 class="text-xs font-semibold text-gold mb-3"><i class="fa-solid fa-location-dot mr-1.5"></i>สถานที่จัดงาน</h3>
        <div class="space-y-2.5">
      ...
      <!-- Date & Time -->
      <section class="bg-navy-light rounded-2xl p-4 border border-gold/10">
        <h3 class="text-xs font-semibold text-gold mb-3"><i class="fa-regular fa-calendar mr-1.5"></i>วันและเวลา</h3>
        <div class="space-y-2.5">
      ...
      <!-- Job Type -->
      <section class="bg-navy-light rounded-2xl p-4 border border-gold/10">
        <h3 class="text-xs font-semibold text-gold mb-3"><i class="fa-solid fa-tag mr-1.5"></i>ประเภทงาน</h3>
        <div class="grid grid-cols-4 gap-2">
          ${JOB_TYPES.map(jt => `
            <button onclick="setJobType('${jt.value}')" id="jobtype-${jt.value}"
              ...
              <span class="text-[9px]">${jt.label}</span>
            </button>
          `).join('')}
        </div>
      </section>
      ...
      <!-- Pricing -->
      <section class="bg-navy-light rounded-2xl p-4 border border-gold/10">
        <h3 class="text-xs font-semibold text-gold mb-3"><i class="fa-solid fa-money-bill-wave mr-1.5"></i>ราคาและมัดจำ</h3>
        <div class="space-y-2.5">
          ...
          <p class="text-xs text-gray-400">ยอดคงเหลือ: <span class="text-gold font-medium" id="remaining-display">${Utils.formatMoney((Number(s.price) || 0) - (Number(s.deposit) || 0))}</span></p>
        </div>
      </section>

      <!-- Equipment -->
      <section class="bg-navy-light rounded-2xl p-4 border border-gold/10">
        <h3 class="text-xs font-semibold text-gold mb-3"><i class="fa-solid fa-boxes-stacked mr-1.5"></i>เลือกอุปกรณ์</h3>
        <div class="space-y-3">
          ${Object.keys(categorized).map(cat => `
            <div>
              <p class="text-[10px] text-gray-500 mb-1.5 uppercase">${CATEGORY_LABELS[cat] || cat}</p>
      ...
      <!-- Remarks -->
      <section class="bg-navy-light rounded-2xl p-4 border border-gold/10">
        <h3 class="text-xs font-semibold text-gold mb-3"><i class="fa-solid fa-note-sticky mr-1.5"></i>หมายเหตุ</h3>
  ...
  return `
    <div class="flex items-center justify-between bg-navy rounded-lg px-3 py-2">
      <label class="flex items-center gap-2 flex-1">
        ...
        <span class="text-xs text-gray-200">${e.name}</span>
      </label>
      ${selected ? `
        <input type="number" min="1" max="${e.availableQty}" value="${selected.qty || 1}"
          onchange="updateEquipmentQty('${e.name}', this.value)"
          class="w-14 bg-navy-light border border-gold/20 rounded px-2 py-1 text-xs text-gray-100 text-center">
      ` : `<span class="text-[10px] text-gray-500">มี ${e.availableQty} ${e.unit}</span>`}
    </div>
  `;
}
...
function toggleEquipment(name, checked) {
  ...
  paintBookingForm(__equipmentCache, []);
  // repaint templates omitted on toggle for performance; refetch not needed since static list unaffected
}
...
function applyTemplate(template) {
  ...
  BawmusicAPI.listTemplates().then(templates => paintBookingForm(__equipmentCache, templates));
  ...
  try {
    ...
    if (conflicts.hasConflict) {
      ...
      const proceed = await Swal.fire({
        ...
        confirmButtonColor: '#d4af37', background: '#1a1a2e', color: '#fff'
      });
